%% Generate a sparse noise stimulus
% a sequence of images where each image shows a gray screen with white and
% black squares. 
...
config = get_setup_config(screen_name, true);

%% calculations

% Calculate the visual angles covered by the screen from the eye position
% This is pure geometry - no warp correction is applied
[x_angle_reg, y_angle_reg] = calculate_viewing_angles(config);

                                                 
% now the screen is divided into a grid of squares
% square info
...
x_border = linspace(0.5, config.w_pix-0.5, n_x+1);
y_border = linspace(0.5, config.h_pix-0.5, n_y+1);
